...
describe('tier2-deep helpers', () => {
  ...
  describe('detectTier2', () => {
    ...
    describe('Integration - Complete Flows', () => {
      ...
      it('should reject authenticator-only 2FA step via channel gate', () => {
        container.innerHTML = `
          <div>
        ...
        // Score passes threshold (75 pts) but channel gate rejects: authenticator-only, no email option
        expect(result.detected).toBe(false)
        expect(result.reason).toContain('Authenticator-only field (channel gate)')
        expect(result.metadata?.layer).toBe('channel-gate')
      })
